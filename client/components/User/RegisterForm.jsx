...
function RegisterForm() {
  ...
  const { currentUser, registerUserLoading, registerUserDone,
    registerUserError, message } = useSelector((state) => state.user);
  const { imagePath } = useSelector((state) => state.image);

  useEffect(() => {
    if (currentUser.id) {
      Router.replace('/');
      Message.warning('로그인 하지 않은 사용자만 접근할 수 있습니다.', 1000);
    }
  }, [currentUser]);
