...
function UserProfile({ User }) {
  ...
  return (
    <Card
      ...
      actions={[
        <div key={uuidv4()} onClick={handleRouter(`/user/${User.id}`)}>
          게시글
          <br />
          {renderCounts(User.Posts)}
        </div>,
        <div key={uuidv4()} onClick={handleRouter('/user/followings')}>
          팔로윙
          <br />
          {renderCounts(User.Followings)}
        </div>,
        <div key={uuidv4()} onClick={handleRouter('/user/followers')}>
          팔로워
          <br />
          {renderCounts(User.Followers)}
        </div>,
      ]}
    >
      <Meta
